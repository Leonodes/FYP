...
def splitloading(bit, root_path, BATCH_SIZE,split_rate=0.9):
    ...
    train_dataset, val_dataset = D.random_split(
        full_dataset, 
        [int(data_num*split_rate), data_num-int(data_num*(split_rate))],
...
def train():
    ...
    for epoch in range(EPOCH):
        ...
        for param_group in optimizer.param_groups:
            lr_disp = param_group['lr']
            
        t_batch_s = time.time()
        for _, (speckles, images) in enumerate(train_loader):
        #for _, (speckles, images) in enumerate(train_loader):
            optimizer.zero_grad()
            train_x = torch.Tensor(speckles[:,:,:scale,:scale]).to(device)
            #train_x = scaling(train_x, scale, order, up='True')
            train_x = torch.sqrt(train_x)
            train_y = torch.Tensor(images).to(device)
            train_y = F.avg_pool2d(train_y,image_down)
            # forward prediction
            ...
            # forward prediction
                loss = loss_func(predict, train_y)/2+loss_func(mts.Gradient_Net()(predict), mts.Gradient_Net()(train_y)).cuda()/2

            # loss_func(mts.Gradient_Net()(predict), mts.Gradient_Net()(train_y)).cuda()/3
            ...
            count += len(train_y)
            t_epoch = time.time()-t_batch_s

            if count % 1000 == 0:
                print('Epoch {} ({}/{})|{:.4f}| Loss: {:.4f}| Time: {:.4f}'.format(
                    epoch+1, count, num_train, lr_disp, loss_train_mean,
                    t_epoch), end='\r'
                )
                f.write('Epoch {} ({}/{})|{:.4f}| Loss: {:.4f}| Time: {:.4f}\n'.format(
                    epoch+1, count, num_train, lr_disp, loss_train_mean, 
                    t_epoch) 
                    )
        ...
        print('Epoch {} ({}/{})|{:.4f}| Loss: {:.4f}| Time: {:.4f}|Test Loss: {:.4f}|PCC: {:.4f}|MSE: {:.4f}|SSIM: {:.4f}|'.format(
            epoch+1, count, num_train, lr_disp, loss_train_mean,
            t_epoch, loss_test_mean, tpcc, tmse, tssim)
        )

        f.write('Epoch {} ({}/{})|{:.4f}| Loss: {:.4f}| Time: {:.4f}|Test Loss: {:.4f}|PCC: {:.4f}|MSE: {:.4f}|SSIM: {:.4f}|\n'.format(
            epoch+1, count, num_train, lr_disp, loss_train_mean,
            t_epoch, loss_test_mean, tpcc, tmse, tssim)
        )
        f.flush()

        if loss_test_mean < best:
...
if __name__ == '__main__':
    ...
    Dataname = 'FFHQ'
    version = '_trial'
    name = './' + str(scale) + Dataname + 'OD' + str(out_dim) + str(bit) + version
    # 记录frame loss帧损失
    f = open(name + '.txt', 'w+')
    # 读取已有DNN参数
    pre_path = ''
...
# Setting Ends
    ...
    print('====> loading datasets')
    train_loader, val_loader, num_train, num_val = splitloading(bit, root_path=root_path, BATCH_SIZE=BATCH_SIZE)
